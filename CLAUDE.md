...
**TTL.** rmcp writes the entry once and never refreshes it while the creating instance serves traffic, so the entry's lifetime is counted from creation, not from last activity. The framework re-arms the TTL on every successful `load` through `PersistenceBackend::touch` — atomic (Redis `EXPIRE`), so a session a peer deleted between the read and the re-arm is reported gone rather than resurrected — and a session that has failed over once stays recoverable on the next failover. A `touch` that errors fails the `load` (fail-closed: the state just read may be a session that no longer exists), which rmcp surfaces as an unknown session — the client re-initializes. A session served by a single instance for longer than the TTL is the remaining gap: it keeps working there, but a failover after that point re-initializes the client — exactly the pre-recovery behaviour, not worse.

Covered by `http_legacy_session_is_restored_on_another_instance` (`tests/http_integration.rs`, two `build_app` instances sharing one `InMemoryBackend`), the unit tests in `session_persistence.rs` (re-arm, delete-between-read-and-touch, touch error → load error, backend without `touch` → never restores) and `touch_re_arms_a_live_key_and_reports_a_deleted_one` against a real Redis.
...
**`touch`**: `PersistenceBackend::touch(ns, key, ttl) -> bool` re-arms an entry's TTL atomically and reports whether it still existed (`RedisBackend`: `EXPIRE`; `InMemoryBackend`: presence check under its lock). The default reports `false` without doing anything: a backend that has not implemented the primitive cannot give the guarantee, so transport session recovery stays **off** for it (every `load` answers "unknown session") instead of failing open. It exists so that recovery never resurrects a deleted session — see the session layer.

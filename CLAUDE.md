...
**Retiring the superseded passthrough grant**: because the key is derived from the access token, a `refresh_token` grant used to store the rotated credentials under a *new* key and leave the previous entry behind. That entry still held the refresh token Keycloak had just rotated away, and the middleware would happily adopt it — refresh, `invalid_grant`, spurious `401`. `/oauth/token` therefore indexes every passthrough grant by `sha256(refresh_token) → grant_key` in `NS_GRANT_REFRESH` (in memory plus persistence write-through, and read-through on a memory miss, the same shape as `NS_OPAQUE_REFRESH`). On a `refresh_token` grant the handler resolves the spent refresh token to the grant it belongs to, removes that entry and its index entry, *then* stores the new one. The removal is skipped when the old key equals the new one — a key derivation stable across rotation (JWT `sid`/`sub`) maps both grants onto the same entry, and deleting it would drop the credentials just written. Covered by `tests/passthrough_grant_cleanup.rs`. Passthrough is slated for deprecation once a ResourceServer mode lands; this is a transition-period mitigation, not a design to build on.
